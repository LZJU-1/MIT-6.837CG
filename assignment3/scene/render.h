...
protected:
          ...
          SceneParser* scene;
public:
          explicit Render(Image *image = nullptr, SceneParser* scene = nullptr){
                    this->image = image;
                    this->scene = scene;
          }
...
private:
          float dMin, dMax;
public:
          explicit DepthRender(Image *image = nullptr, SceneParser *scene = nullptr,
...
public:
          explicit ColorRender(Image *image = nullptr, SceneParser *scene = nullptr) :
            Render(image, scene) {}
...
public:
          explicit NormalRender(Image *image = nullptr, SceneParser *scene = nullptr) :
            Render(image, scene) {}
...
public:
          explicit DiffuseRender(Image *image = nullptr, SceneParser *scene = nullptr, bool shadeBack = false) :
                    ...
                    for(int i = 0; i < n; i++){
                              for(int j = 0; j < m; j++){
                                        ...
                                        for(int k = 0; k < scene->getNumLights(); k++) {
                                                  ...
                                                  Vec3f liDir, liCol;
                                                  float dToLight;
                                                  li->getIllumination(h.getIntersectionPoint(), liDir, liCol, dToLight);
